...
void draw();
int ifTraversed(int r, int c); //判断当前格子是否已经被下了
long hash(int i, int j);
//简单散列算法：将当前状态的棋子数、各棋子的位置顺序编码（1-64）简单相乘
//所谓「不同的状态」的定义就取决于上述散列算法的各个因数：各棋子的位置以及棋子数，所以散列值不可能碰撞
//为"ifTraversed"函数服务
void ClearChessboard(int a[8][8]);                    //清空棋盘
int BACKTRACKING(int chessboard[8][8]);               //核心代码：回溯算法
void PlaceChessman(int a[8][8], int row, int column); //放置棋子
void BackToPreviousState(int a[8][8]);                //返回上一步棋
void HashNode();                                      //计算当前状态的散列值并保存至列表 List_HashedNodes
...
int BACKTRACKING(int chessboard[8][8])
{
    ...
    { //从第1个格子遍历到第64个格子
        ...
        { //每次遍历有四次判断
            ...
            {
                if (row == HistoryRow[k])
                {               //加速运行：对照当前棋盘上各个棋子所在的行
                    column = 7; //如果正在遍历已经有棋子的行，那么将正在遍历的列号调为"7"，下次循环就是下一行
                    break;      //加速运行
                }
            }
            if (!chessboard[row][column])
            { //逻辑判断、如果遍历到的格子是空的（分支 0）
                if (ifTraversed(row, column))
                { //再判断此格子是否已经遍历过，通过散列表（分支 1）
                    if (row == 7 && column == 7)
                    {                                    //如果不仅遍历过而且还是最后一个格子（分支 2）
...
int ifTraversed(int r, int c)
{
    ...
    for (int i = 0; i < Number_HashedNodes; i++)
    { //跟散列表中的元素逐个对比
...
long hash(int i, int j)
{ //散列算法（如果传入行列号参数表示本函数服务于 ifTraversed 函数，用于对比
